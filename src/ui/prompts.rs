use crate::{
    io_utils::{get_trimed_user_input, get_user_input},
...
fn create_epic_prompt() -> Epic {
    println!("----------------------------");
    println!("Epic Name:");
    let name = get_trimed_user_input();
    println!("Epic Description:");
    let description = get_trimed_user_input();
...
fn update_status_prompt() -> Option<Status> {
    ...
    let user_input = get_user_input().trim().parse::<u8>();

    if let Ok(status) = user_input {
        return match status {
            1 => Some(Status::Open),
